...
CreateThread(function()
    while true do
        Wait(500)
        for k,v in pairs(Config.GunsmithLocations) do
            local playerCoords = GetEntityCoords(cache.ped)
            local distance = #(playerCoords - v.npccoords.xyz)

            if distance < Config.DistanceSpawn and not spawnedPeds[k] then
                local spawnedPed = NearPed(v.npcmodel, v.npccoords, v.gunsmithid, v.jobaccess, v.name)
...
function NearPed(npcmodel, npccoords, gunsmithid, jobaccess, name)
        ...
        options = {
            {
                ...
                action = function()
                    TriggerEvent('rex-gunsmith:client:opengunsmith', gunsmithid, jobaccess, name)
                end
            },
        },
        distance = 2.0,
